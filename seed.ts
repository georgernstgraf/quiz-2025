...
async function seed() {
    ...
    const my_qtypes = new Set((await questionService.getAllTypes()).map((qt) => qt.name));
        ...
        while (not_fetched_count > 0 && await questionService.getQuestionCountInCategory(categoryName) < their_catcount) {
            ...
            try {
                ...
                if (response.response_code === 0 && Array.isArray(response.results)) {
                    ...
                    console.log(
                        `   📦 Batch ${batchNumber}: Processed ${batchProcessed} from API, Stored ${batchStored} (${batchProcessed - batchStored
                        } already in database)`,
                    );
                } else {
