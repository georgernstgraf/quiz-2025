...
let cachedCategories = await prisma.category.findMany();
let cachedTypes = await prisma.type.findMany();
...
export async function createQuestion(
  ...
  if (existingQuestion) {
    ...
    for (let i = 0; i < existingIncorrectAnswers.length; i++) {
      if (
        ...
        found_diff = true;
        message += `\nincorrect_answer received: ${
          newIncorrectAnswers[i]
  ...
  const type = cachedTypes.filter((t) => t.name === new_question.type)[0];
  if (!type) {
...
export async function createType(data: TypeData) {
  ...
  cachedTypes = await prisma.type.findMany(); // Refresh cached types
  return result;
}
...
export async function createDifficulty(data: DifficultyData) {
  ...
  cachedDifficulties = await prisma.difficulty.findMany(); // Refresh cached difficulties
  return result;
}
...
export async function createCategory(data: CategoryData) {
  ...
  cachedCategories = await prisma.category.findMany(); // Refresh cached categories
  return result;
}
